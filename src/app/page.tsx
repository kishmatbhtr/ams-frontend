  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="w-5/6 flex flex-col h-screen">
        ...
        <div className="flex p-4 shadow-md border-b-2 justify-end">
          <div className="flex items-center cursor-pointer" onClick={() => { console.log('f'); setShowPanel(!showPanel) }}>
            <Image className="" src={userImg} width={32} alt="alt" /><CaretDownFilled />
          </div>
          <div id="dropdown" className={`${!showPanel ? "hidden" : "absolute"} right-6 top-12 z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-32 border-2 dark:bg-gray-700`}>
            <ul className="py-2">
              <li>
                <a href="/login" className="block px-4 text-lg"><span className="flex items-center"><span className="mx-2 font-semibold">Logout</span> <LogoutOutlined /></span></a>
              </li>
            </ul>
          </div>
        </div>

        {(div == "punchIn") ? <PunchIn /> : (div == "qrCode") ? <QRCode /> : <Report />}
